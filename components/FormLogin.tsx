...
export default function FormLogin() {
  ...
  // Handle OAuth callback saat component mount
  useEffect(() => {
    handleOAuthCallback();
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.status === 'success' && data.authorization) {
            // Store tokens di sessionStorage (konsisten dengan login biasa)
            sessionStorage.setItem('token', data.authorization.token);
            sessionStorage.setItem(
              'token_type',
              data.authorization.type || 'bearer'
          ...
          setError(errorData.message || 'OAuth callback failed');
        }
      } catch (error) {
